...
3. Search for `Rhino_PT`, then click `INSTALL`.
...
1. Open `File`->`Examples`->`Rhino_PT`->`RhinoExample`.
...
1. Compile and upload the `Rhino_PT/GetUUID` sketch from the `File -> Examples` menu. Copy the UUID of the board printed at the beginning of the session to the serial monitor.
